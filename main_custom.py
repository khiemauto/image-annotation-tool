...
class New_Dialog(Ui_new_dialog, QDialog):
    ...
    def check_validity(self):
        ...
        # if len(self.opencsvfile_label) == 0:
...
class Open_Dialog(Ui_open_dialog, QDialog):
    ...
    def check_validity(self):
        ...
        if self.selected_folder_label.text() == '':
            QMessageBox.warning(self, "Warning", "Empty folder path")

        if self.selected_csv_label.text() == '':
            QMessageBox.warning(self, "Warning", "Empty csv path")
        self.accept()
    ...
    def process(self):
        if self.sender() == self.action_new:
            ...
            if ret == QDialog.Accepted:
                ...
                self.labeler_widget = Labeler_Widget(self.new_dialog.label_values, self.new_dialog.selected_folder, self.new_dialog.mode)
            ...
            if ret == QDialog.Accepted:
                ...
                with open(selected_csv) as csvfile:
                    ...
                    if self.labeler_widget is not None:
                        self.labeler_widget.deleteLater()
                    self.labeler_widget = Labeler_Widget(labels, selected_folder, 'csv')
